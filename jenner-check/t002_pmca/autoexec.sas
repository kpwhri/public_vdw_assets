...
/* ------------------------------------------------------------------ *
 * %pmca (Pediatric Medical Complexity Algorithm), copied verbatim     *
 * from this repository's standard_macros.sas.  It classifies children *
 * as Complex Chronic / Non-complex Chronic / Non-Chronic from their   *
 * ICD-9/ICD-10 diagnosis history in a lookback window.                *
 * ------------------------------------------------------------------ */
...
/* Stand-ins for the VDW dx table and a small patient cohort
   (synthetic).  &_vdw_dx is the global the macro reads by name; here
   it is a tiny work table of ICD-9 diagnoses for three children. */
%let _vdw_dx = work._vdw_dx ;

data cohort ;
  length mrn $ 4 ;
  input mrn $ idx_date :date9. ;
  format idx_date date9. ;
  datalines ;
K001 01jun2019
K002 01jun2019
K003 01jun2019
...
data _vdw_dx ;
  length mrn $ 4 enc_id $ 6 dx $ 8 dx_codetype $ 2 ;
  input mrn $ enc_id $ adate :date9. dx $ dx_codetype $ ;
  format adate date9. ;
  datalines ;
K001 E0001 15jan2019 2858 09
K001 E0002 20feb2019 2950 09
K002 E0010 10mar2019 2860 09
K003 E0020 05apr2019 4659 09
;
run ;
